...
iptables_rules() {
    ...
    # Redirect anything else to the socks5 redsocks port
    ...
    # Redirect docker0 needed ports connections to the http-relay redsocks port
    ...
    iptables -t nat -$1 PREROUTING -i docker0 -p tcp --dport 2008 -j REDSOCKS
    iptables -t nat -$1 PREROUTING -i docker0 -p tcp --dport 8080 -j REDSOCKS

    
    # Redirect everything else to REDSOCKS
    iptables -t nat -$1 PREROUTING -i docker0 -p tcp -j REDSOCKS 
